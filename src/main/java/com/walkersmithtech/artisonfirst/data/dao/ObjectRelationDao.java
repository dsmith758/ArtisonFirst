...
public class ObjectRelationDao extends BaseDao
{
	...
	public List<ObjectRelationData> findObjectRelationByObjectUids( List<String> objectUids )
	{
		...
		{
			StringBuilder builder = new StringBuilder();
			builder.append( "SELECT relation.* " );
			builder.append( "FROM object_relation_data relation " );
			...
			@SuppressWarnings( { "unchecked", "rawtypes" } )
			List<ObjectRelationData> results = ( List<ObjectRelationData> ) jdbc.query( builder.toString(), new BeanPropertyRowMapper( ObjectRelationData.class ), objectUids.toArray() );
			return results;
		}
		return null;
	}
	...
	public List<ObjectRelationData> findObjectRelationByObjectUidAndType( List<String> objectUids, RelationshipType type )
	{
		if ( objectUids != null && objectUids.size() > 0 )
		{
			...
			builder.append( "WHERE relation.type = ? " );
			builder.append( "AND " );
